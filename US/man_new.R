...
lagformula <- paste0("lag(",tvconfounders,",2)")
xy <- xy %>% group_by(id) %>% mutate(lagx1= lag(x,1))
xy$y[which(xy$y<0)] <- 0
xy$logy <- log(xy$y+1)
...
##create a corresponding xy
xy.cres <- xy[,c("newy","meantreat15","lagx1",lagc,"id","days","y")]
colnames(xy.cres) <- c("newy","meantreat15","x",tvconfounders,"id","days","newcase")

write.csv(filldata.original,"filldata_original.csv")
write.csv(xy.cres,"xy.csv")

###only look at 45 period after reaching 30 cases
filldata.original %>% group_by(id) %>% slice((match(T,sumcase>=30) ):n())-> filldata.timezero
filldata2 <- filldata.timezero
...
filldata2 %>% group_by(id) %>% filter(days >= mintime & days <= mintime + 44) -> filldata3
filldata <- filldata3

xy <- xy.cres
xy$sumcase <- filldata.original$sumcase
xy %>% group_by(id) %>% slice((match(T,sumcase>=30) ):n()) -> xy2
xy2 %>% group_by(id) %>% mutate(mintime = min(days)) -> xy2
xy2 %>% group_by(id) %>% filter(days >= mintime & days <= mintime + 44) -> xy3
...
filldata.pic <- subset(filldata.original,(Date <= ymd("2020-04-15") & Date >= ymd("2020-03-1")))
...
projectdata <- merge(filldata.pic,xy2,by=c("id","days"))
projectdata <- projectdata[,c("id","days","Date","sumcase.x","treatment","treat0",
                              "treat1","meantreat15","newy","newcase","sumcase_base")]
colnames(projectdata) <- c("id","days","Date","sumcase","treatment","treat0",
                           "treat1","treat_history_15","growth_rate","newcase","sumcase_base")
