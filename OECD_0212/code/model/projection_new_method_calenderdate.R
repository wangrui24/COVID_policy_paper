...
data.full$a1_counter[data.full$Rela.Date30 < 0] <- 
  data.full$Policy.indicator[data.full$Rela.Date30 < 0]
...
treat.all.prop.pre <- c(NA,treat.all.prop[-length(treat.all.prop)])
treat.all.prop.pre[data.full$Date.str2=="2020-01-22"] <- NA
data.full$treat.all.prop.pre <- treat.all.prop.pre

treat.none.prop.pre <- c(NA,treat.none.prop[-length(treat.none.prop)])
treat.none.prop.pre[data.full$Date.str2=="2020-01-22"] <- NA
data.full$treat.none.prop.pre <- treat.none.prop.pre
...
confidence_interval <- function(a_bar, iter = 5000){ 
  ...
  for (i in c(1:iter)) {
    ...
    growth_rate_counter_i <- data.use$New.Growth.rate + 
      beta_i*(a_bar - data.use$Treatment.within14.prop.pre)
    ...
    for (j in c(1:n)) {
      ...
      start.index <- which(data.j$Rela.Date30==0)
      for (k in c(start.index:(t-1))) {
...
predicted_case <- function(a_bar){
  ...
  growth_rate_counter <- data.use$New.Growth.rate + 
    diff*(a_bar - data.use$Treatment.within14.prop.pre)
...
confin_treat_all <- confidence_interval(data.use$treat.all.prop.pre)
confin_treat_all$cumcase_predicted <- predicted_case(data.use$treat.all.prop.pre)
confin_treat_none <- confidence_interval(data.use$treat.none.prop.pre)
confin_treat_none$cumcase_predicted <- predicted_case(data.use$treat.none.prop.pre)
